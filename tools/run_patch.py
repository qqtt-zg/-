...
text = re.sub(
    r'          // \r\n\s+// autoIncrementCheckbox\r\n\s+// \r\n\s+this\.autoIncrementCheckbox\..*?this\.autoIncrementCheckbox\.CheckedChanged.*?;'
    if '\r\n' in text else re'          // \n\s+// autoIncrementCheckbox\n\s+// \n\s+this\.autoIncrementCheckbox\..*?this\.autoIncrementCheckbox\.CheckedChanged.*?;',
    '''            // 
            // cmbOrderNumberMode
            // 
...
with open('src/WindowsFormsApp3/Forms/Main/MaterialSelectFormModern.Designer.cs', 'w', encoding='utf-8') as f:
    text = fix_line_endings = text.replace('\r\n', '\n').replace('niveh', 'niveh')
    f.write(text)
